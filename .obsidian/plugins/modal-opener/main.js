...
var _ModalWindow = class extends import_obsidian2.Modal {
  constructor(plugin, link, file, fragment) {
    super(plugin.app);
    this.observer = null;
  ...
  onClose() {
    if (_ModalWindow.activeInstance && this.plugin.settings.enableRefreshOnClose) {
      const leafContent = _ModalWindow.activeInstance.containerEl.querySelector(".workspace-leaf-content");
      if (leafContent) {
...
var ModalOpenerSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  displayGeneralSettings(containerEl) {
    ...
    if (this.plugin.settings.clickWithoutAlt) {
      new import_obsidian3.Setting(containerEl).setName(t("Only works in read mode")).setDesc(t("Click trigger works only in read mode")).addToggle((toggle) => toggle.setValue(this.plugin.settings.onlyWorksInReadMode).onChange(async (value) => {
...
var _ModalOpenerPlugin = class extends import_obsidian4.Plugin {
  ...
  async onload() {
    ...
    this.documentClickHandler = (evt) => {
      ...
      if (target instanceof HTMLAnchorElement && target.href && this.isValidURL(target.href)) {
        if (altKey && !ctrlKey || this.settings.clickWithoutAlt && !altKey && !ctrlKey && !(this.settings.typeOfClickTrigger === "internal")) {
          evt.preventDefault();
  ...
  duplicateCurrentContentInModal() {
    ...
    const isPinned = leaf == null ? void 0 : leaf.getViewState().pinned;
  ...
  registerAltClickHandler() {
    this.altClickHandler = (evt) => {
      ...
      if (!isAltClick && !this.settings.clickWithoutAlt)
        return;
      if (!(evt.button === 0 && (!evt.ctrlKey || evt.altKey)))
        return;
      if ((_b = target.getAttribute("alt")) == null ? void 0 : _b.endsWith(".svg"))
        return;
      if (this.isPreviewModeLink(target)) {
  ...
  handlePreviewModeLink(evt, isAltClick) {
    ...
    if (linkElement) {
      if (linkElement.hasAttribute("data-tooltip-position") && this.isValidURL(linkElement.href)) {
        target = linkElement;
      }
    ...
    if (!isAltClick) {
      if (this.isValidURL(link)) {
        ...
        if (this.settings.typeOfClickTrigger === "external")
          return;
      }
    }
    evt.preventDefault();
  ...
  handleSourceModeLink(editor, evt, isAltClick) {
    ...
    if (!isAltClick) {
      if (this.settings.onlyWorksInReadMode)
        return;
  ...
  findClosestEmbedElement(element) {
    var _a;
    if (element.classList.contains("canvas-minimap") || element.classList.contains("file-embed-title") || element.classList.contains("markdown-embed-link") || element.closest('.ptl-tldraw-image-container, .dataloom-padding, .dataloom-bottom-bar, [data-viewport-type="element"], svg, rect')) {
      while (element) {
  ...
  getPreviewModeLinkText(target) {
    ...
    return container.getAttribute("data-file-path") || container.getAttribute("data-href") || container.getAttribute("href") || container.getAttribute("data-path") || container.getAttribute("filesource") || container.getAttribute("src") || ((_b = container.textContent) == null ? void 0 : _b.trim()) || "";
  }
  isPreviewModeLink(target) {
    ...
    const closestList = [".annotated-link", ".ge-grid-item"];
  ...
  async openInFloatPreview(link) {
    try {
      ...
      if (!file && !this.isValidURL(link)) {
        new import_obsidian4.Notice(t("The file or link does not valid: ") + link);
        return;
      }
      new ModalWindow(
        this,
